...
int generate_indirect_directory_entry_summary_helper_1(struct ext2_super_block *super_block, int block_number, int index) {
	...
	safe_pread(
		...
		&next_block_number, 
		sizeof(int), 
		block_number * calculate_block_size(super_block) + index * sizeof(int)
	);

	return next_block_number;
}

void generate_indirect_directory_entry_summary_helper_2(struct ext2_super_block *super_block, int block_number, int offset, struct ext2_dir_entry *dir_entry) {
	safe_pread(
		fsi_fd, 
		dir_entry, 
		sizeof(struct ext2_dir_entry), 
		block_number * calculate_block_size(super_block) + offset
	);
}

void generate_single_indirect_directory_entry_summary(struct ext2_super_block *super_block, struct ext2_inode *inode, int inode_number) {
	int i, j, offset, block_number;
	struct ext2_dir_entry dir_entry;

	for (i = 0; i < calculate_num_block_number_in_a_block(super_block); i++) {

		block_number = generate_indirect_directory_entry_summary_helper_1(super_block, inode->i_block[EXT2_IND_BLOCK], i);

		if (block_number == 0) continue; // invalid block reference

		for (j = 0; j < calculate_block_size(super_block); j += dir_entry.rec_len) {

			generate_indirect_directory_entry_summary_helper_2(super_block, block_number, j, &dir_entry);

			if (dir_entry.inode == 0) continue; // invalid inode number

			offset = (EXT2_NDIR_BLOCKS + i) * calculate_block_size(super_block) + j;

			produce_one_directory_entry_info(&dir_entry, inode_number, offset);
...
void generate_double_indirect_directory_entry_summary(struct ext2_super_block *super_block, struct ext2_inode *inode, int inode_number) {
	int i, j, k, offset, indirect_block_number, block_number;
	struct ext2_dir_entry dir_entry;

	for (i = 0; i < calculate_num_block_number_in_a_block(super_block); i++) {

		indirect_block_number = generate_indirect_directory_entry_summary_helper_1(super_block, inode->i_block[EXT2_DIND_BLOCK], i);

		if (indirect_block_number == 0) continue; // invalid block reference

		for (j = 0; j < calculate_num_block_number_in_a_block(super_block); j++) {

			block_number = generate_indirect_directory_entry_summary_helper_1(super_block, indirect_block_number, j);

			if (block_number == 0) continue; // invalid block reference

			for (k = 0; k < calculate_block_size(super_block); k += dir_entry.rec_len) {

				generate_indirect_directory_entry_summary_helper_2(super_block, block_number, k, &dir_entry);

				if (dir_entry.inode == 0) continue; // invalid inode number

				offset = (EXT2_NDIR_BLOCKS + (i + 1) * calculate_num_block_number_in_a_block(super_block) + j) * calculate_block_size(super_block) + k;
...
void generate_triple_indirect_directory_entry_summary(struct ext2_super_block *super_block, struct ext2_inode *inode, int inode_number) {
	int i, j, k, l, offset, double_indirect_block_number, indirect_block_number, block_number;
	struct ext2_dir_entry dir_entry;

	for (i = 0; i < calculate_num_block_number_in_a_block(super_block); i++) {

		double_indirect_block_number = generate_indirect_directory_entry_summary_helper_1(super_block, inode->i_block[EXT2_TIND_BLOCK], i);

		if (double_indirect_block_number == 0) continue;

		for (j = 0; j < calculate_num_block_number_in_a_block(super_block); j++) {

			indirect_block_number = generate_indirect_directory_entry_summary_helper_1(super_block, double_indirect_block_number, j);

			if (indirect_block_number == 0) continue;

			for (k = 0; k < calculate_num_block_number_in_a_block(super_block); k++) {

				block_number = generate_indirect_directory_entry_summary_helper_1(super_block, indirect_block_number, k);

				if (block_number == 0) continue;

				for (l = 0; l < calculate_block_size(super_block); l += dir_entry.rec_len) {

					generate_indirect_directory_entry_summary_helper_2(super_block, block_number, l, &dir_entry);

					if (dir_entry.inode == 0) continue;

					offset = (EXT2_NDIR_BLOCKS + ((i + 1) * calculate_num_block_number_in_a_block(super_block) + 1 + j) * calculate_num_block_number_in_a_block(super_block) + k) * calculate_block_size(super_block) + l;
...
void generate_direct_directory_entry_summary(struct ext2_super_block *super_block, struct ext2_inode *inode, int inode_number) {
	int i, j, offset;
	struct ext2_dir_entry dir_entry;

	// loop through each direct block reference
	for (i = 0; i < EXT2_NDIR_BLOCKS; i++) {

		if (inode->i_block[i] == 0) continue; // invalid block reference

		// loop through each entry in the block
		for (j = 0; j < calculate_block_size(super_block); j += dir_entry.rec_len) {

			generate_indirect_directory_entry_summary_helper_2(super_block, inode->i_block[i], j, &dir_entry);

			if (dir_entry.inode == 0) continue; // invalid inode number

			offset = i * calculate_block_size(super_block) + j;
...
void generate_directory_entry_summary(struct ext2_super_block *super_block, struct ext2_group_desc *group_desc) {
	...
	for (i = 0; i < (int) super_block->s_inodes_count; i++) {
		...
		generate_direct_directory_entry_summary(super_block, &inode, i + 1);

		// single indirect block reference
		if (inode.i_block[EXT2_IND_BLOCK] != 0)  generate_single_indirect_directory_entry_summary(super_block, &inode, i + 1);
		// double indirect block reference
		if (inode.i_block[EXT2_DIND_BLOCK] != 0) generate_double_indirect_directory_entry_summary(super_block, &inode, i + 1);
		// triple indirect block reference
		if (inode.i_block[EXT2_TIND_BLOCK] != 0) generate_triple_indirect_directory_entry_summary(super_block, &inode, i + 1);  
	}
}
...
void generate_single_indirect_block_reference_summary(struct ext2_super_block *super_block, struct ext2_inode *inode, int inode_number) {
	...
	for (i = 0; i < calculate_num_block_number_in_a_block(super_block); i++) {
		...
		generate_indirect_block_reference_summary_helper(super_block, inode->i_block[EXT2_IND_BLOCK], inode_number, offset, i, 1);
...
void generate_double_indirect_block_reference_summary(struct ext2_super_block *super_block, struct ext2_inode *inode, int inode_number) {
	...
	int num_block_number_in_a_block = calculate_num_block_number_in_a_block(super_block);

	for (i = 0; i < num_block_number_in_a_block; i++) {

		offset = EXT2_NDIR_BLOCKS + (i + 1) * num_block_number_in_a_block;

		indirect_block_number = generate_indirect_block_reference_summary_helper(super_block, inode->i_block[EXT2_DIND_BLOCK], inode_number, offset, i, 2);

		if (indirect_block_number == 0) continue;

		for (j = 0; j < num_block_number_in_a_block; j++) {

			offset = EXT2_NDIR_BLOCKS + (i + 1) * num_block_number_in_a_block + j;

			generate_indirect_block_reference_summary_helper(super_block, indirect_block_number, inode_number, offset, j, 1);
...
void generate_triple_indirect_block_reference_summary(struct ext2_super_block *super_block, struct ext2_inode *inode, int inode_number) {
	...
	for (i = 0; i < num_block_number_in_a_block; i++) {
		...
		offset = EXT2_NDIR_BLOCKS + ((i + 1) * num_block_number_in_a_block + 1) * num_block_number_in_a_block;

		double_indirect_block_number = generate_indirect_block_reference_summary_helper(super_block, inode->i_block[EXT2_TIND_BLOCK], inode_number, offset, i, 3);

		if (double_indirect_block_number == 0) continue;

		for (j = 0; j < num_block_number_in_a_block; j++) {

			offset = EXT2_NDIR_BLOCKS + ((i + 1) * num_block_number_in_a_block + 1 + j) * num_block_number_in_a_block;

			indirect_block_number = generate_indirect_block_reference_summary_helper(super_block, double_indirect_block_number, inode_number, offset, j, 2);

			if (indirect_block_number == 0) continue;
			
			for (k = 0; k < num_block_number_in_a_block; k++) {

				offset = EXT2_NDIR_BLOCKS + ((i + 1) * num_block_number_in_a_block + 1 + j) * num_block_number_in_a_block + k;

				generate_indirect_block_reference_summary_helper(super_block, indirect_block_number, inode_number, offset, k, 1);
